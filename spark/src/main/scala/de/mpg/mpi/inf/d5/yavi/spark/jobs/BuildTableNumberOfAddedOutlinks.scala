...
/**
 * BuildTableNumberOfAddedOutlinks
 */
object BuildTableNumberOfAddedOutlinks {
  ...
  def runJob(jobContext: JobContext): Unit = {
    // Load dependencies.
    jobContext.loadTable(AddedOutlinks)

    // Create table as select.
    jobContext.buildTable(NumberOfAddedOutlinks, s"""
      |SELECT
      |  $AddedOutlinks.page_id AS page_id,
      |  $AddedOutlinks.day_number AS day_number,
      |  COUNT(*) AS value
      |FROM $AddedOutlinks
      |GROUP BY $AddedOutlinks.page_id, $AddedOutlinks.day_number
      |SORT BY page_id, day_number
    """.stripMargin)

    // Save newly created table.
    jobContext.saveTable(NumberOfAddedOutlinks)
  }
}
